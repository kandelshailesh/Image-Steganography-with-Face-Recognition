...
var web_host='<div class="dialog" id="{x.id}" style=" height:70px; border-bottom: 1px solid #EEE;"><a style="text-decoration:none; color:black; " href="/chat/{x.id}" id="user{x.id}"><img style="width:30px; height:30px; border-radius:10px; " src="https://frontend-1.adjust.com/new-assets/images/site-images/interface/user.svg"> <span class="title" style="position:relative; top:-10px; font-weight: bolder;">'+'{username}'+'</span><p style="margin-left:30px; max-width: 90%; max-height: 20px;">'+'{message}'+'</p></a></div>';
...
{   
    ...
    $.get('/apis/messages/'+ sender_id + '/' + receiver_id, function (data) {
        ...
        { 
            ...
            var box= web_host.replace('{x.id}',sender_id);
...
function checknewmessage() {
     $.get('/apis/messages/'+ receiver_id, function (data) {
        ...
        if (data.sender !== undefined)
        {
            ...
            var box= web_host.replace('{x.id}',data.sender_id);
            // var box= web_host.replace('{request.user.id}',receiver_id);
